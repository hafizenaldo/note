...
class _NotesScreenState extends State<NotesScreen> {
  ...
  Future<ModelNotes> fetchNotes() async {
    final response = await http.get(Uri.parse('http://192.168.18.25/latihan_notes/getNotes.php'));
  ...
  Future<void> addOrUpdateNote({Datum? note}) async {
    String url = note == null
        ? 'http://192.168.18.25/latihan_notes/addNotes.php'
        : 'http://192.168.18.25/latihan_notes/updateNotes.php';
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonResponse['value'] == 1) {
        ...
        Fluttertoast.showToast(msg: note == null ? 'Note added' : 'Note updated');
      } else {
  ...
  Future<void> deleteNote(String id) async {
    final response = await http.post(
      Uri.parse('http://192.168.18.25/latihan_notes/deleteNotes.php'),
      body: {'id': id},
    );
